...
##### Weighted
```{r, warning=FALSE, message=FALSE}
m2_logit_comp_weighted <- svyglm(infekt_pos ~ female + age + hghinc + hhgr, design=svydesign(id=~1, weights=~phrf_20_final, data=covid_pers), family=quasibinomial(link='logit')) 
...
##### Weighted
```{r, warning=FALSE, message=FALSE}
m3_logit_comp_weighted <- svyglm(infekt_pos ~ female + age + hghinc + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final, data=covid_pers), family=quasibinomial(link='logit')) 
summary(m3_logit_comp_weighted)
```

#### Without income (too much item nonresponse)

##### Unweighted
```{r, warning=FALSE, message=FALSE}
m4_logit_comp <- glm(infekt_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, family = binomial(link='logit'), data = covid_pers)
summary(m4_logit_comp)
```

##### Weighted
```{r, warning=FALSE, message=FALSE}
m4_logit_comp_weighted <- svyglm(infekt_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final, data=covid_pers), family=quasibinomial(link='logit'))
summary(m4_logit_comp_weighted)
```
...
```{r, warning=FALSE, message=FALSE}
DescTools::PseudoR2(m4_logit_comp, which="all")
```
...
library("rms")
rms.out=lrm(infekt_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, data=covid_pers,x=TRUE,y=TRUE)
...
##### Weighted
```{r, warning=FALSE, message=FALSE}
m2_logit_anti_weighted <- svyglm(AK_pos ~ female + age + hghinc + hhgr, design=svydesign(id=~1, weights=~phrf_20_final_dbs, data=covid_pers_AK_only), family=quasibinomial(link='logit'))
...
##### Weighted
```{r, warning=FALSE, message=FALSE}
m3_logit_anti_weighted <- svyglm(AK_pos ~ female + age + hghinc + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final_dbs, data=covid_pers_AK_only), family=quasibinomial(link='logit'))
...
##### Unweighted
```{r, warning=FALSE, message=FALSE}
m4_logit_anti <- glm(AK_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, family = binomial(link='logit'), data = covid_pers)
summary(m4_logit_anti)
```

##### Weighted
```{r, warning=FALSE, message=FALSE}
m4_logit_anti_weighted <- svyglm(AK_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final_dbs, data=covid_pers_AK_only), family=quasibinomial(link='logit'))
summary(m4_logit_anti_weighted)  
```
...
```{r, warning=FALSE, message=FALSE}
DescTools::PseudoR2(m4_logit_anti, which="all")
```
Recommended Pseudo $R^2$ measures: McFadden non-adjusted and adjusted, Tjur's D
...
```{r, warning=FALSE, message=FALSE, fig.height=6, fig.width=6}
library("rms")
rms.out=lrm(AK_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, data=covid_pers,x=TRUE,y=TRUE)
plot(calibrate(rms.out))
```
...
m4_logit_comp_bula <- glm(infekt_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, family = binomial(link='logit'), data = covid_pers_bula)
summary(m4_logit_comp_bula)

m4_logit_anti_bula <- glm(AK_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, family = binomial(link='logit'), data = covid_pers_bula)
summary(m4_logit_anti_bula)
```
...
m4_logit_comp_bula_weighted <- svyglm(infekt_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final, data=covid_pers_bula), family=quasibinomial(link='logit'))
summary(m4_logit_comp_bula_weighted)

m4_logit_anti_bula_weighted <- svyglm(AK_pos ~ female + age + hhgr + open + cons + extra + agree + neuro, design=svydesign(id=~1, weights=~phrf_20_final_dbs, data=covid_pers_AK_only_bula), family=quasibinomial(link='logit'))
summary(m4_logit_anti_bula_weighted)  
```
